...
def coverage(c):
    ...
    """
    c.run("coverage run --source django_renderblock runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django_renderblock.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_renderblock")
...
def lint(c):
    ...
    """
    c.run("flake8 django_renderblock tests")
...
def release(c, bumpsize=''):
    ...
    import django_renderblock
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_renderblock.__version__))
    c.run("git push --tags")
